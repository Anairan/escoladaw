...
@section("formulario")
	...
	<form method="POST" action="/aluno" class="row" enctype =
	"multipart/form-data">
		<div class="form-group col-6">
			<label for="nome">Nome:</label>
			<input type="text" id="nome" name="nome" class=
			"form-control" value="{{ $aluno->nome }}" required
			/>
		</div>
		<div class="form-group col-6">
			<label for="email">Email:</label>
			<input type="text" id="email" name="email" class=
			"form-control" value="{{ $aluno->email }}" 
			required/>
		</div>
		<div class="form-group col-6">
			<label for="curso_id">Curso:</label>
			<select id="curso_id" name="curso_id" class=
			"form-control" required>
				<option value=""></option>
				@foreach ($cursos as $curso)
					<option value="{{ $curso->id }}" @if ($curso->id ==
					$aluno->curso_id) selected @endif>{{
					$curso->nome}}</option>
					@endforeach
			</select>
		</div>
		...
		<tbody>
			@foreach ($alunos as $aluno)
				<tr>
					...
					<td>
					@if($aluno->foto != null)
						<img src='{{str_replace ("public/", "storage/", 
						$aluno->foto) }}' width=
						"300"></img>
					@endif
					</td>
